...
# define a soup obj
soup = bs4.BeautifulSoup(start_url, 'html.parser')
...
def select_location():
    ...
    for links in soup.find_all('span', attrs={'class': 'ws_bold'}):
        names = links.find('ul').find_all('li')
        for link in names.find_all('a'):
            print(link.get('href'))
