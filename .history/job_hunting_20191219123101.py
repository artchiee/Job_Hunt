...
def select_location():
    ...
    for links in soup.find_all('span', attrs={'class': 'ws_bold'}).find('div', attrs={'id': 'LOCATION_rbo'}).find('ul', attrs = {'class': 'rbList'}:)
    liss = links.find_all('li')
    for lis in liss.find('a'):
