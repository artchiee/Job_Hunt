...
def select_location():
    ...
    for links in soup.find_all('div', attrs={'id': 'LOCATION_rbo'}):
        my_list = links.find('ul').find('li')
       # in_list = my_list.find_all('li')
        for all_ in my_list.find_all('a', attrs ={'href': re.compile("^emplois?q=php")}):
            locations_lists.append(all_.get('href'))
            print("Founded urls : ", locations_lists)
