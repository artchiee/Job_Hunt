...
def select_location():
    ...
    for links in soup.find_all('div', attrs={'id': 'LOCATION_rbo'}).find_next('ul'):
        #my_list = links.find('ul')s
        all_ = links.find_next('li')
        for i in all_.find_all('a'):
            my_links =i.get('href')
            print("Founded urls : ",locations_lists.append(my_links))
