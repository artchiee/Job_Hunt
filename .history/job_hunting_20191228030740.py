...
def select_location():
    ...
    div =  soup.find_all('div', attrs = {'id' : 'LOCATION_rbo'})
    ul = div.find('ul').find_next('li')
    for list_ in ul.find_all('a'):
        print(list_.get('href'))



            # Matching usr input to title string  
        
    choice = input('Enter your location to fetch results')
