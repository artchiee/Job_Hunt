...
def save_jobs():
    ...
    try:
        ...
        time.sleep(5)
        # Scrapping the job location from table 
        job_block = all_rs.find_element_by_class_name('recJobLoc')
        block_within = job_block.find_element_by_tag_name('span').text
        if block_within:
            print('\n location   : ', block_within)
        else:
            print Exception()
